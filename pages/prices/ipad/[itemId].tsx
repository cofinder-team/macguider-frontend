...
const IpadModel = ({ newId }: PageProps) => {
  ...
  useEffect(() => {
    amplitudeTrack('enter_price_detail', { type: 'P', id: newId })
  }, [])
  ...
  const {
    ...
    data: currentItem,
  } = useQuery(['item', 'P', itemId], () => getItem<IpadItemResponse>('P', itemId))
  ...
  const {
    ...
    data: currentModel,
  } = useQuery(['model', 'P', itemId], () => getModel('P', currentItem!.model.id), {
    enabled: !!currentItem,
  })
  ...
  return (
    <>
      <PageSEO title={'오늘의 iPad 시세'} description={'중고 아이패드 시세를 알려드립니다'} />
                  ...
                  >
                    <div>
                      ...
                      <p className="font-semibold text-gray-700 dark:text-gray-400">
                        {`${currentItem.details.chip} / ${
                          currentItem.details.cellular ? 'Wi-Fi + Cellular' : 'Wi-Fi'
                        }/ ${currentItem.details.storage}`}
                      </p>
                    </div>
