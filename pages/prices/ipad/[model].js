...
import OptionsModalForIpad from '@/components/OptionsModalForIpad'
...
const IpadModel = ({ model, optionId }) => {
  ...
  switch (model) {
    case 'ipad-mini':
      currentItem = optionsIpad.find((m) => m.id === '6')
      break
    case 'ipad-air':
      currentItem = optionsIpad.find((m) => m.id === '7')
      break
    case 'ipad':
      currentItem = optionsIpad.find((m) => m.id === '8')
      break
    case 'ipad-pro-11':
      currentItem = optionsIpad.find((m) => m.id === '9')
      break
    case 'ipad-pro-12-9':
      currentItem = optionsIpad.find((m) => m.id === '10')
      break
    default:
      currentItem = optionsIpad.find((m) => m.id === '6')
      break
  ...
  const onClickBuyersGuide = useCallback(() => {
    amplitudeTrack('click_route_buyers_guide', { item_class: 'ipad', item_detail: model })
              ...
              >
                <div>
                  ...
                  <p className="font-semibold text-gray-700 dark:text-gray-400">
                    {`${specs.cpu} / ${
                      currentOptionConnectivity === 'wifi' ? 'Wi-Fi' : 'Wi-Fi + Cellular'
                    } / ${currentOptionSsd}`}
                  </p>
                </div>
